...
class Mesh(object):
    ...
    def __init__(self, circles, accuracy=np.array([0.005,4])):
        ...
        self.psi_metric = self._build_psi_metric()
    ...
    def _build_psi_metric(self):
        ...
        psi_metric = np.zeros((self.mesh_length,self.mesh_length))
        for n_i in xrange(self.mesh_length):
            for n_j in self.neighborhoods[n_i]:
                for elem in list(set(self.nodes_to_elements[n_i])&
                    ...
                    psi_metric[n_i,n_j]+=local_psi_metric(
...
mesh = Mesh(np.array([[1,0,0],[0,1,0],[0,0,-1]]), np.array([0.005,6]))
